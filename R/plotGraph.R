...
#' @name plotGraph
#'
#' @param x either an object of class \code{BayesMVP} (default) or a symmetric
...
plotGraph <- function(x, Pmax = 0.5, main = "Estimated graph of responses",
  ...
  if (!inherits(x, "BayesMVP")) {
    if (is.matrix(x) && is.numeric(x)) {
      if (!((dim(x)[1] == dim(x)[2]) && (sum(dim(x)) > 2))) {
        stop("Use only with a \"BayesMVP\" object or numeric square matrix")
      }
      Gy_hat <- x
  ...
  net <- graph_from_adjacency_matrix(Gy_thresh,
    weighted = TRUE,
