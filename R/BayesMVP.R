#' @title Fitting BayesMVP models
#' @description
#' Main function of the package. Fits a range of models introduced in the
#' package vignette \code{BayesMVP.pdf}. Returns an object of S3 class
#' \code{BayesMVP}. There are three options for the prior on the residual
...
#' }
#'
#' @return An object of class \code{BayesMVP} is saved as
#' \code{obj_BayesMVP.RData} in the output file, including the following
#' components:
#' \itemize{
...
#' @references Zhao Z, Banterle M, Bottolo L, Richardson S, Lewin A, Zucknick M (2021). \emph{BayesMVP: An R package for high-dimensional multivariate Bayesian variable and covariance selection in linear regression.} Journal of Statistical Software, 100: 1–32.
...
#'
#' @export
BayesMVP <- function(data = NULL, Y, X, X_0 = NULL,
  ...
  if (dataLength == 0) {
    my_stop(
      "Please provide a correct path to a plain-text (.txt) file",
      tmpFolder
    )
  }
  ...
  ret$status <- BayesMVP_internal(
    data, mrfG, blockList, structureGraph,
  ...
  ## save fitted object
  obj_BayesMVP <- list(
    status = ret$status, input = ret$input,
    output = ret$output, call = ret$call
  )
  save(obj_BayesMVP, file = paste(sep = "", outFilePath, "obj_BayesMVP.RData"))
