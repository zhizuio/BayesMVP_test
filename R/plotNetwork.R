...
#' from an object of class \code{BayesMVP}
...
plotSEMgraph <- function(ADJmatrix,
  ...
  # igraph objects
  graphADJ <- graph.adjacency(as.matrix(ADJmatrix),
    weighted = TRUE,
    diag = FALSE, mode = "undirected"
  )
  graphSEM <- graph.adjacency(as.matrix(semgraph),
    weighted = TRUE,
