...
#' @description
#' Extract the posterior mean of the parameters of a \code{BayesMVP} class object.
#' @name getEstimator
#'
...
#'
#' @return Return the estimator from an object of class \code{BayesMVP}. It is
...
getEstimator <- function(object, estimator = "gamma", Pmax = 0,
  ...
  if ("beta" %in% estimator) {
    ...
    if ("X0" %in% names(object$output)) {
      rownames(ret$beta) <-
        c(
          colnames(read.table(object$output$X0, header = TRUE)),
  ...
  if ("CPO" %in% estimator) {
    if (is.null(object$output$CPO)) {
      stop("Please specify argument output_CPO in BayesMVP()!")
    }
