...
#' fitted model generated by \code{BayesMVP} which is a \code{BayesMVP} object.
  ...
  if (ncol(CPO) > 1) {
    ...
    if (outlier.mark) {
      if (min(CPO) > outlier.thresh) {
        ...
        text(
          rep(CPO_idx, times = ncol(CPO))[
            which(as.vector(CPO) <= outlier.thresh)
          ] + mark.pos[1],
          as.vector(CPO[CPO < outlier.thresh]) + mark.pos[2],
          labels = rep(name.responses,
            each = nrow(CPO)
          )[as.vector(CPO) < outlier.thresh],
    ...
    if (scale.CPO) CPO <- CPO / max(CPO)
    plot.default(CPO ~ seq_along(CPO),
      xaxt = "n", bty = "n",
      xlim = c(1, length(CPO)),
